export default () => [
  {
    id: 5,
    slug: 'how-i-got-into-vido-game-collecting',
    title: 'How I Got Into Video Game Collecting',
    tag_line: `The story of a guy that got to witness the evolution of video games from NES to current day.`,
    created_at: '02/25/2019',
    pinned: false,
    intro:
      'The story of how I came to know video games as a kid, strayed from them as a teenager, and ultimately became a passionate collector and player as an adult.',
    image:
      'https://res.cloudinary.com/https-joeyg-me/image/upload/v1552515487/gaming/early_collection.jpg',
    alt: `earlier photo of Joey Gauthier's game collection`,
    meta: {
      keywords:
        'gaming, nes, retro games, genesis, dreamcast, retro game collecting, vintage video games, apple iic'
    }
  },
  {
    id: 6,
    slug: 'the-struggle-of-hookinh-it-all-up',
    title: `Connecting All of Those Consoles`,
    tag_line: `Don't just collect em, play em too!`,
    created_at: '03/02/2019',
    pinned: false,
    intro: `An overview of how I keep all of my consoles connected to my 4K TV for the best picture possible on a budget.`,
    image:
      'https://res.cloudinary.com/https-joeyg-me/image/upload/v1552515483/gaming/cable_mess.jpg',
    alt: `image of messy cables and power strips`,
    meta: {
      keywords:
        'gaming, nes, retro games, genesis, dreamcast, retro game collecting, vintage video games, upscaler, switcher, SCART, component cable, RGB'
    }
  },
  {
    id: 7,
    slug: 'how-i-add-to-my-collection',
    title: `How I Add to My Collection`,
    tag_line: `Getting good deals can take some work!`,
    created_at: '03/07/2019',
    pinned: true,
    intro: `Finding good deals on retro gaming items can be tricky! This is a high-level look at how I approach things.`,
    image:
      'https://res.cloudinary.com/https-joeyg-me/image/upload/v1552515483/gaming/flea-market.jpg',
    alt: `image of cluttered tables at a flea market`,
    meta: {
      keywords:
        'gaming, nes, retro games, retro collecting, Game Stop, Goodwill, Game Over Video Games, retro game stores, garage sales, flea markets, pricecharting'
    }
  },
  {
    id: 8,
    slug: 'how-i-store-my-games-collection',
    title: `How I Store My Games`,
    tag_line: `Protect those old games while making them look good on your shelves at the same time!`,
    created_at: '03/13/2019',
    pinned: true,
    intro: `You want to make sure your games are well protected. This is a look at how to achieve that while also making them look better on your shelves.`,
    image:
      'https://res.cloudinary.com/https-joeyg-me/image/upload/v1552515484/gaming/custom_cases.jpg',
    alt: `image of retro games from various consoles in custom cases`,
    meta: {
      keywords:
        'gaming, nes, retro games, retro collecting, universal game cases, ds cases, game storage, cover project, umd cases, dvd cases, cd cases'
    }
  }
];
